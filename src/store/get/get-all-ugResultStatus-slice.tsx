...
interface UgResultStatusType {
  ...
  isRun: string;
  resetActions: string;
  responseForUgResultStatus: getValuesType[];
}
...
export const getUgResultStatusValues = createAsyncThunk<any>("getAllUgResultStatus", async (_, { rejectWithValue }) => {
  try {
    const response = await coreLeadCaptureApi.get("api/crm/lead/leadacademicdetailsug");
...
const getAllUgResultStatusSlice = createSlice({
  name: "ugResultStatus/getAllUgResultStatus",
  initialState,
  reducers: {
  ...
  extraReducers(builder) {
    builder
      ...
      .addCase(getUgResultStatusValues.rejected, (state, action) => {
        state.isLoading = false;
        state.isError = action.error.message || "An Error occured while getting Ug ResultStatus data";
      });
  },
});

export const { resetActionsForUgResultStatusFormField, takeActionsForUgResultStatusFormField } = getAllUgResultStatusSlice.actions;
