...
import { twelfthResultStatusType } from "../../types/twelfth-result-status-type";
...
interface TwelfthResultStatusType {
  ...
  isRun: string;
  resetActions: string;
  responseForTwelfthResultStatus: twelfthResultStatusType[];
}
...
export const getTwelfthResultStatusValues = createAsyncThunk<any>(
  "getAllTwelfthResultStatus",
  async (_, { rejectWithValue }) => {
    try {
      const response = await coreservicesApi.get(
        "api/crm/core/coretwelveresultstatus"
      );
  ...
  extraReducers(builder) {
    builder
      ...
      .addCase(getTwelfthResultStatusValues.fulfilled, (state, action) => {
        ...
        state.isRun = uuidv4();
        state.responseForTwelfthResultStatus = action.payload.map(
          (item: any) => ({
      ...
      .addCase(getTwelfthResultStatusValues.rejected, (state, action) => {
        ...
        state.isError =
          ...
          "An Error occured while getting Twelfth ResultStatus data";
      });
  },
});

export const {
  resetActionsForTwelfthResultStatusFormField,
  takeActionsForTwelfthResultStatusFormField,
} = getAllTwelfthResultStatusSlice.actions;
export const getAllTwelfthResultStatusReducer =
  getAllTwelfthResultStatusSlice.reducer;
