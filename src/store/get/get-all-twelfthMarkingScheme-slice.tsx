...
interface TwelfthMarkingSchemeType {
  ...
  isRun: string;
  resetActions: string;
  responseForTwelfthMarkingScheme: getValuesType[];
}
...
export const getTwelfthMarkingSchemeValues = createAsyncThunk<any>("getAllTwelfthMarkingScheme", async (_, { rejectWithValue }) => {
  try {
    const response = await coreservicesApi.get("api/crm/core/coretwelvemarkingscheme");
  ...
  extraReducers(builder) {
    builder
      ...
      .addCase(getTwelfthMarkingSchemeValues.fulfilled, (state, action) => {
        ...
        state.isRun = uuidv4();
        state.responseForTwelfthMarkingScheme = action.payload.map((item: any) => ({
      ...
      .addCase(getTwelfthMarkingSchemeValues.rejected, (state, action) => {
        state.isLoading = false;
        state.isError = action.error.message || "An Error occured while getting TwelfthMarkingScheme data";
      });
  },
});

export const { resetActionsForTwelfthMarkingSchemeFormField, takeActionsForTwelfthMarkingSchemeFormField } = getAllTwelfthMarkingSchemeSlice.actions;
export const getAllTwelfthMarkingSchemeReducer = getAllTwelfthMarkingSchemeSlice.reducer;
