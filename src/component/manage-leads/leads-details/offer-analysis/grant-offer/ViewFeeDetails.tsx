...
const ViewFeeDetails: React.FC<propsType> = ({ isNewOffer }) => {
  ...
  useEffect(() => {
    if (
      ...
      setFeeDetailsData(
        transformLeadHistoryFeeData(leadOfferHistoryByOfferIdResponse)
      );
    }
  }, [
    ...
    <>
      {feeDetailsdata.length !== 0 && (
        <>
                ...
                {feeDetailsdata.slice(0, 3).map((ele) => (
                  <div className="flex items-start" key={ele.id}>
                    ...
                    <div className="px-4 py-1 w-1/2 flex justify-end">
                      <div
                        className={`flex justify-between w-full max-w-[110px] ${ele.title === "Total Course Fee"
                          ? "font-semibold"
                          : ""
                          }`}
                      >
                  ...
                  <div className="flex items-start" key={ele.id}>
                    ...
                    <div className="px-4 py-1 w-1/2 flex justify-end">
                      ...
                      >
                        {ele.id !== 4 && (
                          <span
                            className={` ${ele.title === "Total Discount"
                              ? "font-semibold"
                              : ""
                              }`}
                          >
          ...
          {(isNewOffer) && <div className="flex justify-end mt-5">
            <button
