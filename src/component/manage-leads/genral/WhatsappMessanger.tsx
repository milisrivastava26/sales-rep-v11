...
import {
  ...
  resetWhatsappTemplateById,
} from "../../../store/whatsapp -messenger/get-whatsappTemplate-by-templateId-slice";
import { IoLogoWhatsapp } from "react-icons/io";
import Select from "react-select";
import { useParams } from "react-router-dom";
import { SingleValue } from "react-select";
import { sendWhatsappByTemplateId } from "../../../store/whatsapp -messenger/send-whatsapp-slice";
import { sendBulkWhatsappByTemplateId } from "../../../store/whatsapp -messenger/send-bulk-whatsapp-slice";
...
const WhatsAppMessenger: React.FC<propsType> = ({ isMode, data }) => {
  ...
  const { isLoading: isLoadingForAlltemplates, whatsappTemplates } =
    useSelector((state: RootState) => state.getAllWhatsappTemplate);

  const { isLoading: isLoadingForTemplateData, whatsappTemplateById } =
    useSelector((state: RootState) => state.getWhatsappTemplateByTemplateId);
  const { isLoading: isLoadingSendWhatsapp } = useSelector(
    (state: RootState) => state.sendWhatsapp
  );
  const { isLoading: isLoadingForBulk } = useSelector(
    (state: RootState) => state.sendBulkWhatsapp
  );
  ...
  const handleSend = () => {
    if (isMode === "bulkUpdate") {
      const leads = data?.map((item: any) => ({
        leadCaptureId: item.leadCaptureId,
        toNumber: item.phone,
      }));
      ...
      store.dispatch(sendBulkWhatsappByTemplateId(payload));
    } else {
      const payload = {
        ...
        <form className="space-y-6 mb-12">
          ...
          <div className="text-center absolute right-2 bottom-4">
            <button
              type="button"
              onClick={handleSend}
              disabled={isLoadingSendWhatsapp || isLoadingForBulk || whatsappTemplateById === ""}
              className={`${
                isLoadingSendWhatsapp || isLoadingForBulk || whatsappTemplateById === ""
                  ? "bg-opacity-50"
                  : "hover:bg-blue-700"
              } inline-flex items-center gap-2 bg-blue-600 text-white font-medium px-4 py-2 rounded-md transition-colors`}
            >
              Send
            </button>
          </div>
