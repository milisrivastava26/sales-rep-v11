...
import { resetResponseForLeadOwnerUpdate, updateLeadOwner } from "../../../store/sales-rep-details(changeOwner)/update-lead-owner-by-id-slice";
...
const ChangeOwner: React.FC<changeOwnerProps> = ({ leadCaptureId, onHideModal }) => {
  ...
  const { isError, isLoading, responseLeadOwnerUpdate } = useSelector((state: RootState) => state.updateLeadOwner);
  ...
  useEffect(() => {
    if (!isError && !isLoading && Object.keys(responseLeadOwnerUpdate).length !== 0) {
  ...
  return (
    <div className="p-4">
      ...
      <button
        type="submit"
        className={`bg-blue-600 text-white px-4 py-2 rounded absolute bottom-[16px] right-[16px] ${isLoading ? "bg-opacity-50 cursor-not-allowed" : ""}`}
        onClick={changeOwner}
        disabled={isLoading}
      >
        Save
