...
const BulkChangeOwner: React.FC<changeOwnerProps> = ({ onHideModal, isMode }) => {
  const { currentURL } = useForLocation();
  const { getAllCheckSelectedDataFormCustomTable, getLeadsForManageTask } = useSelector((state: RootState) => state.ui);

  const key = isMode === "advanceSearch" ? "leadCaptureId" : "leadCaptureId";
  const leadCaptureIds = (getAllCheckSelectedDataFormCustomTable ?? []).map((item: { lead_capture_id: number; leadCaptureId?: number }) => {
      ...
      <form>
        <div className="grid w-full pb-5">
          ...
          {((leadCaptureIds.length === 0 && isMode === "bulkUpdate") || (leadCaptureIdForTask.length === 0 && isMode === "manageTask")) && (
            <span className="text-sm text-red-500 my-2">Please Select atleast on lead</span>
          )}
        </div>
      </form>

      <button
        disabled={(isMode === "bulkUpdate" && leadCaptureIds.length === 0) || (leadCaptureIdForTask.length === 0 && isMode === "manageTask")}
        type="submit"
        className={`${
          leadCaptureIdForTask.length === 0 && isMode === "manageTask" ? "bg-opacity-50 cursor-not-allowed" : ""
