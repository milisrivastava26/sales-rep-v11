...
const FilterHeadV2: React.FC<PropsType> = ({ inputData, filterpayload, setFilterPayload }) => {
  ...
  const handleMenuOpen = (name: string) => {
    if (name === "current_lead_stage_display_name") {
      ...
      dispatch(getleadSubStagesById(selectedValues.current_lead_stage_display_name?.value));
              ...
              return (
                <div key={field.id} className="flex flex-col gap-y-1">
                  ...
                  <Select
                    ...
                    isClearable
                    isDisabled={field.name === "current_salesrep_full_name" && !userDetails?.authority?.some((role: string) => ["ROLE_ADMIN", "ROLE_MANAGER"].includes(role))}
                    styles={{
                      control: (base) => ({
                        ...base,
                        minHeight: "27px",
