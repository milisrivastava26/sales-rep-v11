...
const ImportLeadApi = axios.create({

  baseURL: `${baseURL}:4040`,
});
// Request interceptor to add the Authorization header
ImportLeadApi.interceptors.request.use(
  async (config) => {
...
ImportLeadApi.interceptors.response.use(
  (response) => response,
  async (error: AxiosError) => {
    ...
    if (error.response?.status === 401 && !originalRequest?._retry) {
      ...
      if (newToken) {
        ...
        return ImportLeadApi(originalRequest);
      }
    }
