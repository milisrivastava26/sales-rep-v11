...
const manageLeadsApi = axios.create({
  baseURL: `${baseURL}:7076`,
  timeout: 50000,
});
// Request interceptor to add the Authorization header
manageLeadsApi.interceptors.request.use(
  async (config) => {
...
manageLeadsApi.interceptors.response.use(
  (response) => response,
  async (error: AxiosError) => {
    ...
    if (error.response?.status === 401 && !originalRequest?._retry) {
      ...
      if (newToken) {
        ...
        return manageLeadsApi(originalRequest);
      }
    }
