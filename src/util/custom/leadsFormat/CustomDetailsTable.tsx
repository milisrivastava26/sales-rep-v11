...
import {
  ...
  resetLeadPaymentDetails,
} from "../../../store/paymentInfo/get-leadPaymentDetails-byOrderId-slice";
import {
  getCrmLeadPaymentDetailsByOrderId,
  resetCrmLeadPaymentDetails,
} from "../../../store/paymentInfo/get-crmLeadPaymentDetails-slice";
import {
  getLeadFailedPaymentDetailsByPaymentId,
  resetFailedPaymentDetails,
} from "../../../store/paymentInfo/get-leadFailedPaymentDetails-slice";
...
export function CustomDetailsTable<T extends object>({
  ...
  const { isLoading: isLoadingForPostDiscountToPs } = useSelector(
    (state: RootState) => state.postDiscountToPs
  );
  ...
  const { paginatedProps, paymentDetailsPageSize } = useSelector(
 ...
 p-2 text-left text-sm text-nowrap`}
                    >
                      {cell.column.Header === "Recording" ? (
                        ...
                        isMode === "ManageTicket" ? (
                        <span className="block w-[250px] overflow-hidden text-ellipsis whitespace-normal [-webkit-line-clamp:3] [-webkit-box-orient:vertical]">
                          {cell.render("Cell")}
                        </span>
                        ...
                        <p
                          ...
                          onClick={() => {
                            if (row.original.status === "captured") {
                              ...
                              store.dispatch(
                                setRazorPayPaymentId(row.original.id)
                              );
                              store.dispatch(
                                getCrmLeadPaymentDetailsByOrderId(
                        ...
                        <span
                          className="text-blue-500 text-medium underline underline-offset-2 cursor-pointer"
                          onClick={() => {
                            navigate(`/manage-ticket/${cellValue}`);
                          }}
                        >
                          {cell.render("Cell")}
                        </span>
